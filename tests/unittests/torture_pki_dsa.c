...
static void torture_pki_sign_data_dsa(void **state)
{
    ...
    /* Test using SHA1 */
    rc = test_sign_verify_data(key, SSH_DIGEST_SHA1, template, sizeof(template));
    assert_int_equal(rc, SSH_OK);
...
static void torture_pki_fail_sign_with_incompatible_hash(void **state)
{
    ...
    /* Sign the buffer */
    sig = pki_sign_data(key, SSH_DIGEST_SHA1, template, sizeof(template));
    assert_non_null(sig);

    /* Verify signature */
    rc = pki_verify_data_signature(sig, pubkey, template, sizeof(template));
    assert_int_equal(rc, SSH_OK);

    /* Test if signature fails with SSH_DIGEST_AUTO */
    bad_sig = pki_sign_data(key, SSH_DIGEST_AUTO, template, sizeof(template));
    assert_null(bad_sig);
    ...
    bad_sig = pki_sign_data(key, SSH_DIGEST_SHA256, template, sizeof(template));
    assert_null(bad_sig);
    ...
    bad_sig = pki_sign_data(key, SSH_DIGEST_SHA384, template, sizeof(template));
    assert_null(bad_sig);
    ...
    bad_sig = pki_sign_data(key, SSH_DIGEST_SHA512, template, sizeof(template));
    assert_null(bad_sig);
...
static void torture_pki_dsa_generate_key(void **state)
{
    ...
    assert_int_equal(rc, SSH_OK);
    assert_non_null(pubkey);
    sign = pki_do_sign(key, template, sizeof(template), SSH_DIGEST_SHA1);
    assert_non_null(sign);
    rc = ssh_pki_signature_verify(session, sign, pubkey, template, sizeof(template));
    assert_true(rc == SSH_OK);
...
static void torture_pki_dsa_cert_verify(void **state)
{
    ...
    sign = pki_do_sign(privkey, template, sizeof(template), SSH_DIGEST_SHA1);
    assert_non_null(sign);
    rc = ssh_pki_signature_verify(session, sign, cert, template, sizeof(template));
    assert_true(rc == SSH_OK);
    ssh_signature_free(sign);
    SSH_KEY_FREE(privkey);
    SSH_KEY_FREE(cert);
