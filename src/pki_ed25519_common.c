...
int pki_ed25519_key_dup(ssh_key new_key, const ssh_key key)
{
    ...
    if (key->ed25519_privkey != NULL) {
        ...
        /* In OpenSSL implementation, the private key is the original private
         * seed, without the public key. */
        new_key->ed25519_privkey = (uint8_t*)malloc(ED25519_KEY_LEN);
#else
        /* In the internal implementation, the private key is the concatenation
         * of the private seed with the public key. */
    ...
    if (key->ed25519_pubkey != NULL) {
        new_key->ed25519_pubkey = (uint8_t*)malloc(ED25519_KEY_LEN);
...
int pki_signature_from_ed25519_blob(ssh_session session, ssh_signature sig, ssh_string sig_blob)
{
    ...
    if (len != ED25519_SIG_LEN){
        SSH_LOG_COMMON(session, SSH_LOG_WARN, "Invalid ssh-ed25519 signature len: %zu", len);
        return SSH_ERROR;
    }
