...
int ssh_gssapi_handle_userauth(ssh_session session, const char *user, uint32_t n_oid, ssh_string *oids){
    ...
    if (maj_stat != GSS_S_COMPLETE) {
        SSH_LOG_COMMON(session, SSH_LOG_WARNING, "indicate mecks %d, %d", maj_stat, min_stat);
    ...
    for (i=0; i < supported->count; ++i){
        ...
        SSH_LOG_COMMON(session, SSH_LOG_DEBUG, "Supported mech %zu: %s", i, ptr);
        free(ptr);
    }
    ...
    for (i=0 ; i< n_oid ; ++i){
        ...
        if(len < 2 || oid_s[0] != SSH_OID_TAG || ((size_t)oid_s[1]) != len - 2){
            SSH_LOG_COMMON(session, SSH_LOG_WARNING,"GSSAPI: received invalid OID");
            continue;
        }
    ...
    if (oid_count == 0){
        SSH_LOG_COMMON(session, SSH_LOG_PROTOCOL,"GSSAPI: no OID match");
    ...
    if (maj_stat != GSS_S_COMPLETE) {
        SSH_LOG_COMMON(session, SSH_LOG_WARNING, "importing name %d, %d", maj_stat, min_stat);
        ssh_gssapi_log_error(SSH_LOG_WARNING,
                             "importing name",
                             maj_stat,
                             min_stat);
    ...
    if (maj_stat != GSS_S_COMPLETE) {
        SSH_LOG_COMMON(session, SSH_LOG_WARNING, "error acquiring credentials %d, %d", maj_stat, min_stat);
        ssh_gssapi_log_error(SSH_LOG_WARNING,
                             "acquiring creds",
                             maj_stat,
                             min_stat);
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PROTOCOL, "acquiring credentials %d, %d", maj_stat, min_stat);
    ...
    for (i=0 ; i< n_oid ; ++i){
        ...
        if(present){
            SSH_LOG_COMMON(session, SSH_LOG_PACKET, "Selected oid %zu", i);
            break;
        }
    }
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_gssapi_token_server){
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PACKET,"Received SSH_MSG_USERAUTH_GSSAPI_TOKEN");
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PACKET, "GSSAPI Token : %s",hexa);
    SAFE_FREE(hexa);
    ...
    if (output_token.length != 0){
        ...
        SSH_LOG_COMMON(session, SSH_LOG_PACKET, "GSSAPI: sending token %s",hexa);
        SAFE_FREE(hexa);
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_gssapi_mic)
{
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PACKET,"Received SSH_MSG_USERAUTH_GSSAPI_MIC");
...
static int ssh_gssapi_match(ssh_session session, gss_OID_set *valid_oids)
{
    ...
    for (i = 0; i < actual_mechs->count; i++) {
        ...
        if (maj_stat == GSS_S_COMPLETE && lifetime > 0) {
            ...
            SSH_LOG_COMMON(session, SSH_LOG_DEBUG, "GSSAPI valid oid %d : %s", i, ptr);
            SAFE_FREE(ptr);
        }
    }
...
int ssh_gssapi_auth_mic(ssh_session session){
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PROTOCOL, "Authenticating with gssapi to host %s with user %s",
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PROTOCOL, "Sending %zu oids", n_oids);
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_gssapi_response){
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PACKET, "Received SSH_USERAUTH_GSSAPI_RESPONSE");
...
static int ssh_gssapi_send_mic(ssh_session session){
    ...
    SSH_LOG_COMMON(session, SSH_LOG_PACKET,"Sending SSH_MSG_USERAUTH_GSSAPI_MIC");
