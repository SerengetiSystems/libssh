...
#ifdef DEBUG_CRYPTO
  SSH_LOG(SSH_LOG_CRYPTO, SIZET_SPECIFIER " bits, " SIZET_SPECIFIER " bytes, " SIZET_SPECIFIER " padding", bits, len, pad);
#endif /* DEBUG_CRYPTO */
...
#ifdef DEBUG_CRYPTO
  SSH_LOG(SSH_LOG_CRYPTO, "Importing a " SIZET_SPECIFIER " bits, " SIZET_SPECIFIER " bytes object ...",
      len * 8, len);
#endif /* DEBUG_CRYPTO */
...
void ssh_print_bignum(const char *name, const_bignum num)
{
    ...
    SSH_LOG(SSH_LOG_CRYPTO, "%s value: %s", name, (hex == NULL) ? "(null)" : (char *)hex);
